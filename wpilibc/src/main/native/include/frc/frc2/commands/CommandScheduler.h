...
/**
 * The scheduler responsible for running Commands.  A Command-based robot should call Run() on the singleton instance in its periodic block in order to run commands
 * synchronously from the main loop.  Subsystems should be registered with the scheduler using
 * RegisterSubsystem() in order for their Periodic() methods to be called and for their default commands to be scheduled.
 */
class CommandScheduler final : public frc::SendableBase, frc::ErrorBase {
 public:
   ...
   * IsFinished() method should return false), as they would simply be re-scheduled
  ...
  /**
   * Whether a given command is running.  Note that this only works on commands that are
   * directly scheduled by the scheduler; it will not work on commands inside of CommandGroups, as
   * the scheduler does not see them.
   *
   * @param commands the command to query
