...
public class Pose2d {
   ...
   * [x_new]    [cos, -sin, 0][transform.x] [y_new] += [sin,  cos, 0][transform.y] [t_new]    [0,
   * 0,   1][transform.t]
   *
   ...
   * stabilization algorithms to get the error between the reference and the current pose.
   *
   ...
   * Controls Engineering in the FIRST Robotics Competition</a> section on nonlinear pose estimation
   * for derivation.
   *
   * <p>The twist is a change in pose in the robot's coordinate frame since the
   * previous pose update. When the user runs exp() on the previous known field-relative pose with
   * the argument being the twist, the user will receive the new field-relative pose.
   *
   ...
   *              For example, if a non-holonomic robot moves forward 0.01 meters and changes angle
   *              by .5 degrees since the previous pose update, the twist would be Twist2d{0.01,
   *              0.0, toRadians(0.5)}
